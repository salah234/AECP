...
Mirrors coordinator/app/channels.py and agents/app/channels.py's
file-based cert loading for the client side, falling back to an
insecure channel when no mTLS files are configured (dev/local, matching
build_server's own insecure fallback in grpc_server.py), and always
attaches a caller-id metadata entry so the allow-list on the far end can
authorize the call even when no verified mTLS peer identity is available
(see aecp_platform.identity.AllowList's interim metadata fallback).
"""
