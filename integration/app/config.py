...
class Settings:
    ...
    def from_env(cls) -> "Settings":
        ...
        grpc_port = int(require("INTEGRATION_GRPC_PORT"))
        http_port = int(require("INTEGRATION_HTTP_PORT"))

        if not (1 <= grpc_port <= 65535):
            raise ValueError("INTEGRATION_GRPC_PORT NOT IN BOUNDS")
        if not (1 <= http_port <= 65535):
            raise ValueError("INTEGRATION_HTTP_PORT NOT IN BOUNDS")
