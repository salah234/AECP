"""Ownership boundary overlap detection for the scheduler.

Deliberately self-contained: services never import each other's internal
modules (only proto/gRPC crosses a service boundary — see CLAUDE.md's key
invariant), so this mirrors taskgraph/app/ownership.py's glob-intersection
algorithm as Coordinator's own copy, operating directly on
aecp.common.v1.OwnershipBoundary protobuf messages (which expose the same
path_globs/forbidden_globs repeated fields taskgraph's pydantic model
does) rather than importing taskgraph's schema type.
"""
...
def path_within_boundary(path: str, boundary: _OwnershipBoundaryLike) -> bool:
...
def violating_paths(paths: list[str], boundary: _OwnershipBoundaryLike) -> list[str]:
    """Return the subset of `paths` that fall outside `boundary`."""
    return [path for path in paths if not path_within_boundary(path, boundary)]
...
def _effective_path_globs(boundary: _OwnershipBoundaryLike) -> list[str]:
    return [
        path_glob
        for path_glob in boundary.path_globs
        if not any(
...
def _globs_may_intersect(a_glob: str, b_glob: str) -> bool:
    ...
    def intersects(a_index: int, b_index: int) -> bool:
        ...
        if a_segment == "**":
            return intersects(a_index + 1, b_index) or intersects(a_index, b_index + 1)

        if b_segment == "**":
            return intersects(a_index, b_index + 1) or intersects(a_index + 1, b_index)
...
def _known_excluded_intersection(
    a_glob: str,
    b_glob: str,
    a: _OwnershipBoundaryLike,
    b: _OwnershipBoundaryLike,
) -> bool:
    return any(_glob_covers(forbidden_glob, b_glob) for forbidden_glob in a.forbidden_globs) or any(
        _glob_covers(forbidden_glob, a_glob) for forbidden_glob in b.forbidden_globs
    )
...
def _has_matching_overlap_sample(
    ...
    return any(
        path_within_boundary(path, a) and path_within_boundary(path, b)
    ...
    for segment_glob in _split_glob(path_glob):
        if segment_glob == "**":
            samples = [
                ...
                for sample in (prefix, (*prefix, "sample"))
            ]
            continue

        segment = _sample_segment_for_glob(segment_glob)
        samples = [(*prefix, segment) for prefix in samples]
...
def _segment_globs_may_intersect(a_segment: str, b_segment: str) -> bool:
    ...
    if fnmatch.fnmatchcase(a_sample, a_segment) and fnmatch.fnmatchcase(a_sample, b_segment):
        return True

    b_sample = _sample_segment_for_glob(b_segment)
    if fnmatch.fnmatchcase(b_sample, b_segment) and fnmatch.fnmatchcase(b_sample, a_segment):
        return True

    a_prefix = _literal_prefix(a_segment)
    b_prefix = _literal_prefix(b_segment)
    if a_prefix and b_prefix and not a_prefix.startswith(b_prefix) and not b_prefix.startswith(a_prefix):
        return False

    a_suffix = _literal_suffix(a_segment)
    b_suffix = _literal_suffix(b_segment)
    if a_suffix and b_suffix and not a_suffix.endswith(b_suffix) and not b_suffix.endswith(a_suffix):
        return False
    ...
    if covering_segments and covering_segments[-1] == "**":
        ...
        return all(
            ...
            for covering_segment, covered_segment in zip(
                prefix_segments, covered_segments[: len(prefix_segments)]
            )
        )
