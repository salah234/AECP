...
Agent Pool is the first client-side consumer of a generated service stub
in this repo. Kept intentionally minimal: typed methods for exactly the
calls Agent Pool needs today (record_decision, used by handoff.py) plus
get_ownership/get_interface_contract for future use once
proto/state/v1/state.proto grows a query-by-task RPC (see
hydration.py's documented gap) — not a generic pass-through of every
StateService RPC.
"""
