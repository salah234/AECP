"""Observability service settings, layered on aecp_platform.config.Loader."""
...
class Settings:
    ...
    def from_env(cls) -> "Settings":
        ...
        grpc_port = int(require("OBSERVABILITY_GRPC_PORT"))
        http_port = int(require("OBSERVABILITY_HTTP_PORT"))

        if not (1 <= grpc_port <= 65535):
            raise ValueError("OBSERVABILITY_GRPC_PORT NOT IN BOUNDS")
        if not (1 <= http_port <= 65535):
            raise ValueError("OBSERVABILITY_HTTP_PORT NOT IN BOUNDS")
